...
CuttingPlaneDolloOld::CuttingPlaneDolloOld(SATSolver* solver, const Matrix& B, const int m, const int n, const int k, StlIntMatrix& B2Var, StlBoolMatrix& activeEntries, Matrix& solA)
  : CuttingPlane(solver)
  , _B(B)
  , _m(m)
  , _n(n)
...
int CuttingPlaneDolloOld::getEntryAssignment(int p, int c){
...
int CuttingPlaneDolloOld::separate()
{
  int nrNewClauses = 0;
...
void CuttingPlaneDolloOld::processSolution(){
  for (int p = 0; p < _m; p++){
    for (int c = 0; c < _n; ++c){
